pipeline {
  ...
  stages {
    ...
    stage("Docker pull") {
      steps {
        withCredentials([usernameColonPassword(credentialsId: '968507ef-f956-4c94-bf2a-cfc03445607c', variable: 'DOCKER')]) {
        sh 'docker login --username ajaydocker21 --password-stdin < ~/@Ajay$IND21'
